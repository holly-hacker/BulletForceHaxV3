...
You can download the WebGL game files by inspecting network traffic through your browser's developer tools or a HTTP(S) proxy.
...
The location of these files may (and likely will) change in the future. To ensure you have the latest game files, look for them in dev tools yourself.

### Bullet Force APK (Android build)

You can get a copy of the Bullet Force APK from [APKMirror](https://www.apkmirror.com/?s=bullet+force). You may find multiple variants even within a version number, but pick the one that was uploaded last. You can expect this file to be ~500mb in size.
...
The current Photon Server SDK (v5.x.x.x and later) seems to only be accessible to paying [Photon Circle Members](https://www.photonengine.com/gaming/pricing) which starts at $125/month with a minimum initial term of 3 months.
